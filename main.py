...
def checker(txt_username_value , txt_password_value, username, password, role_no, file_location): #Function which take argument from function which store username, password, txt_username_value , txt_password_value of specific role and check their username and password.
    for i in range (2,-1,-1):
        if username == txt_username_value and password == txt_password_value:
            ...
            if i > 0:
                ...
                role_checker(role_no, file_location)
            else:
...
def role_checker(role_no, file_location): #Function which read administator txt file and retrive username and password and send them to checker function
    username, password = login()
    with open(file_location, "r") as file: #Open file which user want to login.
        ...
        for i in range(len(lines)):
            if lines[i].strip().startswith(f"username:{username}"): #Ex: if user found in 3rd line it take username and also take password from 4th line.
                ...
                checker(txt_username_value, txt_password_value, username, password, role_no, file_location) #Send Username and Password stored in file to Checker Function.
        checker("", "", username, password, role_no, file_location) #If username and Password not found Send blank in username and password.
...
def roles(): #Function which ask user, their role of getting access.
    ...
    if role_no == 1:
        role_checker(role_no, file_location="Username and Password/Administrator.txt")
    elif role_no == 2:
        role_checker(role_no, file_location="Username and Password/Manager.txt")
    elif role_no == 3:
        role_checker(role_no, file_location="Username and Password/Chef.txt")
    elif role_no == 4:
        ...
        while True: #to let ask user value again and again if they do not enter value 1-2
            if cos_login == 1:
                role_checker(role_no, file_location="Username and Password/Customer.txt")
                break
            elif cos_login == 2:
                customer_register(role_no, file_location="Username and Password/Customer.txt")
                break
            else:
